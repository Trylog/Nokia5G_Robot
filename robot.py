...
class Communication:
    ...
    def __init__(self, robotId, host="192.168.100.88", port=9090):
    ...
    def convert_mes_to_table(self, input_string):
        ...
        for message in messages:
            if message:
                ...
                event_table = list(map(int, tables[0].split(',')))
                control_table = list(map(int, tables[1].split(',')))
        ...
        while True:
            ...
            for i in range(6):
                if self.com.control_table[i] > 0:
                    ...
                    #self.mc.send_coord(i, self.coords[i] + 2, 20)
                    self.coords[i] += 2
                elif self.com.control_table[i] < 0:
                    self.mc.jog_angle(i + 1, 0, abs(self.com.control_table[i]))
                    #self.mc.send_coord(i, self.coords[i] - 2, 20)
                    self.coords[i] -= 2
